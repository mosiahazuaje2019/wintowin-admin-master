...
export class MarcaComponent implements OnInit {
  ...
  add(){
    const time = Date.now().toString();
    this.corek.socket.emit('query',{'event':time +"queryS", 'querystring':"SELECT * FROM `wp_posts` WHERE `post_content` LIKE '"+this.form.value.name+"' AND post_type LIKE 'marcas'"});
    this.corek.socket.on(time + 'queryS', (data , key )=>{
      if(data.length > 0){
        alert('La marca que ingresas ya existe');
      }else{
        this.corek.socket.emit('insert_post',{'condition': {'post_type': 'marcas', 'post_content':this.form.value.name},'key':'insertpost','event':time + 'insert'});
  ...
  edit(idMarca){
    this.compareId = 0;
    const time = Date.now().toString();    
    this.corek.socket.emit('query',{'event':time +"queryS", 'querystring':"SELECT * FROM `wp_posts` WHERE `post_content` LIKE '"+this.formNewName.value.newName+"' AND post_type LIKE 'marcas'"});
    this.corek.socket.on(time + 'queryS', (data , key )=>{
      if(data.length > 0){
        alert('La marca que ingresas ya existe');
      }else{
        ...
        this.corek.socket.on(time + 'updateP', (data , key )=>{
          alert('La marca ha sido actualizada');
          this.process();
        });        
      }
    });
